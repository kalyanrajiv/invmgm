...
<?php
	...
	if(!isset($mobileCostData)){
		...
		$totalMobileCost = $CURRENCY_TYPE.round($mobileCostData['0']['total_mobile_cost'],2);
	}
?>
...
<div class="mobileReSales index">
	<?php //pr($this->request);die;
		$qryStrArr = array();
		$value = $queryStr = $adminKiosk = '';
		if(!empty($this->request->data)){
			$adminKiosk = $this->request->data['MobileResale']['kiosk_id'];
		}
		$kiosk_id = $this->request->Session()->read('kiosk_id');
		if(!isset($start_date)){$start_date = 0;}else{$qryStrArr[]="start_date=$start_date";}
		if(!isset($end_date)){$end_date = 0;}else{$qryStrArr[]="end_date=$end_date";}
 ...
 ?>
	...
	<a href="/mobile-re-sales/export_cost/?<?php echo implode('&',$qryStrArr);?>" target='_blank' title='export csv'><?php echo $this->Html->image('/img/export.png', array('fullBase' => true));?></a>
	<div id='printDiv'>
    ...
    //pr($mobileReSale);die;
		...
		if($mobileReSale->refund_status==1){
			...
			$refundSellEntry[$id] = $mobileReSale->refund_price;
			//pr($refundSellEntry);
?>
	...
	<?php } ?>
		<td><?php echo $id; ?>&nbsp;</td>
		<?php if($this->request->session()->read('Auth.User.group_id') || $this->request->session()->read('Auth.User.group_id') == MANAGERS){?>
		...
		<td><?php
				if($mobileReSale->refund_status==1){
                    $refund_date = $mobileReSale->refund_date;
                     $refund_date->i18nFormat(
                                                                   [\IntlDateFormatter::FULL, \IntlDateFormatter::FULL]
                                                                   );
					$refund_date_with_seconds = $refund_date->i18nFormat('dd-MM-yyyy HH:mm:ss');
					echo date('d-m-y g:i', strtotime($refund_date_with_seconds));
				}
		?>&nbsp;</td>
		<?php
		...
		?>
		<td><?php echo date('jS M, Y g:i A',strtotime($created_date));//$this->Time->format('d-m-y g:i', $created,null,null); ?>&nbsp;</td>
		<td>
...
<?php endforeach;
	...
	?>
		<tr>
			...
			<td><strong><?php echo $CURRENCY_TYPE.$totalCostPrice;//echo $currency.$totalRefundPrice; ?></strong></td>
		...
		<tr>
			...
			<td colspan=2><strong><?php echo $show." = ".$CURRENCY_TYPE.$totalProfit; ?></strong></td>
		</tr>
	</tbody>
	</table>
	<p>
