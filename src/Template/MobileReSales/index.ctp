	...
	<?php echo $this->Form->create('Mobile_re_sales',array('url' => $webRoot,'type' => 'get'));?>
	 ...
	 <div class="search_div">
		...
		<?php
                ...
                <td><input type='button' name='reset' value='reset' style = "width:140px" onClick='reset_search();'/></td>
                    </tr>
                </table>
			</div>
			<?php
            ...
            if(count($this->request->query) && array_key_exists('submit',$this->request->query)){
				...
				if(empty($start)){
                    ...
                    if(array_key_exists('MobileResale',$this->request->query)){
                        $kiosk = $this->request->query['MobileResale']['kiosk_id'];
                    }else{
                        $kiosk = '';
                    }
...
<div class="actions">
	...
	<ul>
		<li><?php echo $this->Html->link(__('List Mobile <br/> Purchases'), array('controller'=>'mobile_purchases','action' => 'index'),['escape'=>false]); ?></li>
