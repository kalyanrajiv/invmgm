...
?>	
<h2><?php echo __('Mobile Re Sale'); //pr($mobileReSale);die; ?></h2>
<?php if((($this->request->session()->read('Auth.User.group_id') == ADMINISTRATORS || $this->request->session()->read('Auth.User.group_id') == FRANCHISE_OWNER) && ($mobileReSale['kiosk_id']==0 || $mobileReSale['kiosk_id']==10000)||
           ...
           <?php }?>
			<?php echo $this->Html->link(__('Sale Receipt'), array('action' => 'mobile_sale_receipt', $mobileReSale['id'])); ?><?php if($this->request->session()->read('Auth.User.group_id') == ADMINISTRATORS || $this->request->session()->read('Auth.User.group_id') == FRANCHISE_OWNER || $this->request->session()->read('Auth.User.group_id') == MANAGERS){ ?>
			...
			<?php echo $this->Html->link('Thermal receipt',array('controller' => 'prints','action'=>'mobile-sale',$mobileReSale['id'])); ?>
	...
	<dl>
		...
		<dd>
			<?php
			if($mobileReSale['kiosk_id'] == 0){
				$kiosk_id = 10000;
			}else{
				$kiosk_id = $mobileReSale['kiosk_id'];
			}
			echo $this->Html->link($kiosks[$kiosk_id], array('controller' => 'kiosks', 'action' => 'view', $kiosk_id)); ?>
			&nbsp;
		</dd>
	</dl>
...
<h4><?php echo __('Mobile Detail'); ?></h4>
	<dl>
		...
		<dd>
			<?php echo $this->Html->link($mobileReSale['brand']['brand'], array('controller' => 'brands', 'action' => 'view', $mobileReSale['brand']['id'])); ?>
			&nbsp;
		</dd>
		<dt><?php echo __('Model'); ?></dt>
		<dd>
			<?php
            if(array_key_exists($mobileReSale['mobile_model_id'],$mobileModels)){
                echo h($mobileModels[$mobileReSale['mobile_model_id']]);    
            }
             ?>
			&nbsp;
		</dd>
		...
		<dd>
			...
			if(array_key_exists('network_id',$networks)){
				 echo $networks[$mobileReSale['network_id']];
			}
			?>
			&nbsp;
		</dd>
		...
		<dd>
			<?php
            if(array_key_exists($mobileReSale['type'],$type)){
                echo h($type[$mobileReSale['type']]);   
            }
             ?>
			&nbsp;
		</dd>
		...
		<dd>
			...
			if($mobileReSale['discount']>0){
				echo h($discountOptions[$mobileReSale['discount']]);
			}else{
				echo "--";
			}
			 ?>
		...
		<?php if(!empty($mobileReSale['discounted_price'])){
			$discountedPrice = $mobileReSale['discounted_price'];
...
<h4><?php echo __('Miscellaneous'); ?></h4>
	<dl>
		...
		<dd>
			<?php
            if(array_key_exists($mobileReSale['status'],$status)){
                echo $status[$mobileReSale['status']];   
            }
             ?>
			&nbsp;
		</dd>
		...
		<dd>
			<?php echo date('jS M, Y g:i A',strtotime($mobileReSale['created']));//$this->Time->format('jS M, Y g:i A',$mobileReSale['created'],null,null); ?>
			&nbsp;
		</dd>
		<dt><?php echo __('Modified'); ?></dt>
		<dd>
			<?php echo date('jS M, Y g:i A',strtotime($mobileReSale['modified']));//$this->Time->format('jS M, Y g:i A',$mobileReSale['modified'],null,null); ?>
			&nbsp;
		</dd>
	</dl>
</div>
