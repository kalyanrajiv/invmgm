...
<div class="mobileReSales index">
	...
	<?php
        ...
        if(!empty($this->request->query['MobileResale']['kiosk_id'])){
			$idKiosk = $this->request->query['MobileResale']['kiosk_id'];
		}else{
			 $idKiosk = '';
		}
        //pr($adminKiosk);die;
	?>
	<?php $webRoot = $this->request->webroot."MobileReSales/search";?>
	<?php echo $this->Form->create('Mobile_re_sales',array('url' => $webRoot,'type' => 'get'));?>
	 ...
	 <div class="search_div">
			...
			<div>
                <table>
                    <tr>
                        <td><input type = "text" id='datepicker1' readonly='readonly' name = "start_date" placeholder = "From Date" style = "width:155px;margin-top: 30px;" autofocus value='<?php echo $start_date;?>' /></td>
                        <td><input type = "text" id='datepicker2' readonly='readonly' name = "end_date" placeholder = "To Date" style = "width:155px;margin-top: 30px;" value='<?php echo $end_date;?>' /></td>
                        <?php 
                 ...
                 //pr($kioskId);die;
				  if(!empty($idKiosk)){
				   echo $this->Form->input(null, array(
					     ...
					     'id'=> 'kioskid',
					      'value' => $idKiosk,
					      'name' => 'MobileResale[kiosk_id]',
					     'empty' => 'Select Kiosk',
					     'default' => $adminKiosk,
					     'style' => 'margin-top: 9px;width:160px;'
					    )
					 );
				  }else{
				    echo $this->Form->input(null, array(
				...
				<?php  }  ?>
                <td><input type = "submit" value = "Search Mobile Re-Sales" name = "submit",style = "width:155px;margin-top:30px;"/></td>
                <td><input type='button' name='reset' value='reset' style = "width:155px;" onClick='reset_search();'/></td>
                    </tr>
                </table>
			...
			<?php
            //pr($totalMobileRefund);die;
            //pr($this->request->query);die;
            if(count($this->request->query) && array_key_exists('submit',$this->request->query)){
				$netSale = $saleSum-$totalMobileRefund;
				?>
				<?php $start = $this->request->query['start_date'];
					$last = $this->request->query['end_date'];
                    //pr($this->request);die;
                    if(array_key_exists('data',$this->request->query)){
                        $kiosk = $this->request->query['data']['MobileResale']['kiosk_id'];
                    }else{
                        $kiosk = '';
                    }
				?>
			<span style="float: left; font-weight : bold">Gross Sale = &#163;<?=$saleSum?>, Refund = &#163;<?=$totalMobileRefund;?>, Net Sale = &#163;<?=$netSale;?></span>
			<span style="float: right; font-weight : bold;">Estimate Cost of sold phones = &#163 <?php echo $this->Html->link($totalMobileCost, array('action' => 'research',$start,$last,$kiosk),array('target'=>'_blank','escape'=>false)); ?><?//=$totalMobileCost;?></span>
			<div>
				<span style="float: left;background: yellow;">For exact cost of sold phone click above on "estimate cost of sold phones" link</span>
			</div>
			<?php } ?>
		</fieldset>
	</div>
 ...
 ?>
	<h2><?php echo __('Mobile Sales'); ?>&nbsp;
    <?php if(array_key_exists('submit',$this->request->query)){ ?>
    <a href="<?php echo $rootURL;?>MobileReSales/export/?<?php echo $queryStr;?>" target='_blank' title='export csv'><?php echo $this->Html->image('/img/export.png', array('fullBase' => true));?></a>
    <?php }else{ ?>
    <a href="<?php echo $rootURL;?>MobileReSales/export/?<?php echo $queryStr;?>" target='_blank' title='export csv'><?php echo $this->Html->image('/img/export.png', array('fullBase' => true));?></a>
    <?php } ?>
    </h2>
...
?>
	...
	<?php } ?>
		...
		<td><?php echo h($modelName[$mobileReSale->mobile_model_id]); ?>&nbsp;</td>
		<td><?php echo h($colorOptions[$mobileReSale->color]); ?>&nbsp;</td>
		...
		<td>
			...
			?>
			<?php echo $this->Html->link($this->Html->image($editUrl,array('fullBase' => true)), array('action' => 'edit', $id),array('escapeTitle' => false, 'title' => 'Edit', 'alt' => 'Edit'));?>
			<?php echo $this->Html->link($this->Html->image($viewUrl,array('fullBase' => true)), array('action' => 'view', $id), array('escapeTitle' => false, 'title' => 'View', 'alt' => 'View'));?>
			<?php echo $this->Html->link($this->Html->image($cloneUrl,array('fullBase' => true)), array('action' => 'update_resale_payment', $id), array('escapeTitle' => false, 'title' => 'Update Payment', 'alt' => 'Update Payment'));?>
...
<?php endforeach;
	...
	?>
		...
		<tr>
			...
			<td>&nbsp;</td>
			<?php } ?>
			<td colspan='4'><strong>Sale excluding VAT(<?php echo $CURRENCY_TYPE.number_format($vatAmount,2);//echo $currency.number_format($vatAmount,2);?>):</strong></td>
			<td><strong><?php echo $CURRENCY_TYPE.number_format($netAmount,2);//echo $currency.number_format($netAmount,2); ?></strong></td>
		</tr>
		<tr>
			<?php if($this->request->session()->read('Auth.User.group_id') == ADMINISTRATORS || $this->request->session()->read('Auth.User.group_id') == FRANCHISE_OWNER){?>
			<td>&nbsp;</td>
			...
			<td><strong><?php echo $CURRENCY_TYPE.number_format($totalRefundGain,2);//echo $currency.number_format($totalRefundGain,2); ?></strong></td>
		</tr>
		<tr>
