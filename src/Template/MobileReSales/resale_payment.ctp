...
<?php
//pr($this->Session->read('resale_data_session'));
	if(is_array($this->request->Session()->read('resale_data_session'))){
		$resaleData = $this->request->Session()->read('resale_data_session.MobileReSale');
...
<?php $after_payment_redirect_print = $this->Url->build(array('controller'=> 'prints', 'action' => 'mobile_sale'));?>
...
<?php $cancel_redirect = $this->Url->build(array('controller'=> 'mobile_re_sales', 'action' => 'add',$purchase_id));?>
