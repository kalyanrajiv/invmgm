...
<table style='width:350px;'>
	<tr>
		<td><span style=''><h2><?php echo __('Payment2'); ?></h2></span></td>
		<td><input type='radio' id = 'full_or_part_1' name='full_or_part' value='1' checked = 'checked' onClick='full_Part(1);'></td><td>Full Payment1</td>
...
<table id="main_table">
	...
	Total : <input type="text" name="total" readonly id="total" value='0.00' style="width: 55px;height: 25px;margin-bottom: 10px;" disabled='disabled'/> Invoice Amount:<span id="invoice_amount"><?php //echo $amount;?></span> <strong>Due Amount:</strong><span id ='due_amount'><?php echo $amount;?></span>
...
<input type="button" name='submit1' value="Submit" class = 'final_step_pay' style="margin-top: 16px;"/>
</td>
				<td><input type="submit" id="cancel_pyment" name='cancel' value="Cancel" style="margin-top: 16px;"/></td>
			</tr>
		</table>
	</div>
...
<input type='hidden' name='repiar_id' id='repiar_id' value='' />
<script>
...
<script>
    $(document).on('click','#cancel_pyment',function(){
        $('#payment_screen').hide();
        $('#product').show();
    });
	...
	$(document).on('click','.final_step_pay',function(){
		...
		var repair_id = document.getElementById('repiar_id').value;
		if (repair_id == "" || repair_id == "undefined") {
			alert('repair id is missing');
            return false;
        }
