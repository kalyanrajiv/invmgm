...
<?php
	if(is_array($this->request->Session()->read('unlock_data_session'))){
		$finalAmount = $this->request->Session()->read('unlock_data_session.MobileUnlock.estimated_cost');
	}elseif(is_array($this->request->Session()->read('manager_edit_data'))){
		$finalAmount = $this->request->Session()->read('manager_edit_data.MobileUnlock.estimated_cost');
	}
