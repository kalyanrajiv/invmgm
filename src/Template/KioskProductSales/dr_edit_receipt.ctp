...
<?php
        ...
        $controller = "kiosk-product-sales"; 
		$webRoot = $this->request->webroot.$controller."/dr_search_edit_receipt/$orderId";
...
<?php
	echo $this->Form->create(null, array('url' => array('controller' => 'kiosk_product_sales',
														'action' => 'dr_save_updated_receipt'),'autocomplete'=>'off'));
    ...
    <table cellpadding="0" cellspacing="0">
        <thead>
			<tr>
				...
				<th><?php echo $this->Paginator->sort('id','Product'); ?></th>
				<th>Category</th>
	...
	$hiddenPageNum = ""; // sourabh
		foreach ($products as $key => $product){
			...
			$truncatedProduct = \Cake\Utility\Text::truncate($product['product'], 30, ['ellipsis' => '...', 'exact' => false]);
...
<script type="text/javascript">
    <?php
		foreach ($products as $key => $product):
			...
			echo "\njQuery('#tooltip_{$product['id']}').tooltip({content:\"".htmlspecialchars($string)."\",track:true});\n";
