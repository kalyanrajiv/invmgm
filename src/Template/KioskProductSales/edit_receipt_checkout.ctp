			...
			foreach($session_basket as $key => $basketItem){
				...
				$basketStr.="<tr>
						...
						<td>".$basketItem['product']."</td>
					 	...
					 	<td>".$currency.$price_without_vat  ."</td>
						
						<td>".$currency.(round($discountAmount,2))."</td>
						<td>".$currency.(round($totalItemCost,2) ) ."</td>
						<td>".$this->Html->link('delete',array('action'=>'delete_product_from_session2',$key,$orderId),array('id'=>$key,'onClick'=>'return reply_click(this.id)','value'=>'delete','name'=>'delete_product'))."</td>
						</tr>";
			}
			if(!empty($basketStr)){
				$basketStr = "<table><tr>
							...
							<tr><td colspan='6'>Bulk Discount ({$bulkDiscountPercentage}%)</td><td>".$currency.round($bulkDiscountValue,2)."</td></tr>
							<tr><td colspan='6'>Sub Total</td><td>".$currency.(round($netBillingAmount,2) )."</td></tr>
							<tr><td colspan='6'>Vat</td><td>".$currency.(round($vatAmount,2) )."</td></tr>
							<tr><td colspan='6'>Total Amount</td><td>".$currency.(round($finalAmount,2) )."</td></tr>
							<tr><td>&nbsp;</td>
							<td>&nbsp;</td>
							<td>&nbsp;</td>
							<td>&nbsp;</td>
							...
							<td><span class='actions'>".$this->Html->link('Sell',array('action'=>'save_updated_receipt',$orderId),array('name'=>'submit'))."</span></td>
