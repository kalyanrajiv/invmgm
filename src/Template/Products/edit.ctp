...
<?php
//pr($this->request);
$deleteImage = $this->Url->build(["controller" => "products","action" => "deleteImage"],true); ?>
<input type='hidden' name='deleteImage' id='deleteImage' value='<?=$deleteImage?>' />
<?php $maximumDiscountedValue = "";
echo $this->Form->create($product,array('enctype' => 'multipart/form-data')); ?>
	...
	//	 pr($this->request['data']);
		...
		if(@readlink($absoluteImagePath) || file_exists($absoluteImagePath)){
			...
			if(!empty($image)){
				echo $this->Form->input('delete', array('type' => 'checkbox', 'label' => 'Delete existing image'));
			}else{
				echo $this->Form->input('delete', array('type' => 'checkbox', 'label' => 'Delete existing image'));
			}
		}
	...
	<?php
	//echo" For replacing image; in first edit attempt case image will be deleted if existing and in 2nd edit attempt case when new image will be uploaded, it would be linked to product. ";
		echo $this->Form->input('Product.remove', array('type' => 'hidden', 'label' => 'Remove existing file'));//'type' => 'checkbox',
	...
	$('#delete').click(function() {
		if(this.checked){
			...
			if (box==true){
				...
				var targeturl = $("#deleteImage").val();
				targeturl += "?product_id="+product_id;
				//"http://hpwaheguru.co.uk/products/deleteImage?product_id="+product_id;
				var image = "/products/edit/"+product_id;
				$.blockUI({ message: 'Just a moment...' });
				$.ajax({
					type: "get",
					url: targeturl,
					success: function(response) {
						$.unblockUI();
						$('#delete').hide();
						$("label:contains('Delete existing image')").hide();
						$("a[href='"+image+"']").hide();
						//alert(response(0));
