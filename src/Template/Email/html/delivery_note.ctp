...
$address1 = $address2 = $city = $state = $postalCode = $phone_num = "";
...
if($customer_table[0]['id']>0){
    ...
    if($customer_table[0]['mobile']){
		$phone_num = $customer_table[0]['mobile'];
	}
    ...
    if($productReceipt['mobile']){
		$postalCode = $productReceipt['mobile'];
	}
}
//pr($sale_table);die;
?>
...
<table border="1" cellspacing="0" style="width: 700px;">
	...
	<tr>
			...
			<table style="text-align: center;float: right; width:482px;">
				<tr>
					<td style="font-size: 15px;"><strong><?php if(isset($invoice)){ echo "Invoice"; }else{ echo  "Delivery Note"; } ?><?php if((int)$sale_table[0]['kiosk_id']){
	...
	<tr style="width: 100%;">
		...
		<td colspan='5' style="width: 45%;"><strong>Product</strong></td>
		<td style="width: 10%;" colspan="1"><strong>Quantity</strong></td>
	</tr>
	<?php
	$amount = 0;
	$totalVat = 0;
	...
	foreach($sale_table as $key => $product){
		...
		<tr style="width: 100%;">
			...
			<td colspan='5' style="width: 45%;"><?= $productName[$product['product_id']];?></td>
			<td style="width: 10%;" colspan="1"><?= $qty;?></td>
		</tr>
	<?php 	}
	}
