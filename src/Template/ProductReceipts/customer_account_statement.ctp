<?php
	...
	if(isset($startDate) &&!empty($startDate)){
		foreach($recptInOrders as $key => $recptInOrder){
			...
			$outStandingSum+=$orig_bill_amount;
			$pmtSum+=$amount;
			...
			switch( $montStr){
				case date('m-Y'):
					$oneMonStandSum += $orig_bill_amount;
					$oneMonPmtSum += $amount;
					break;
				case date('m-Y', mktime(0, 0, 0, date('m')-1, 1, date('Y'))):
					$twoMonStandSum += $orig_bill_amount;
					$twoMonPmtSum += $amount;
					break;
				case date('m-Y', mktime(0, 0, 0, date('m')-2, 1, date('Y'))):
					$threeMonStandSum += $orig_bill_amount;
					$threeMonPmtSum += $amount;
					break;
				case date('m-Y', mktime(0, 0, 0, date('m')-3, 1, date('Y'))):
					$fourMonStandSum += $orig_bill_amount;
					$fourMonPmtSum += $amount;
					break;
				default:
					$fiveMonStandSum += $orig_bill_amount;
					$fiveMonPmtSum += $amount;
			}
...
<table  style='width: 850px;'>
	<tr>
		<td width='33%' style='text-align:left;'><?php echo $logoImg;?></td>
	...
	<tr>
		<td width='33%' valign='top'><?php echo $customerStr;?></td>
		
		<td width='33%' style="text-align: right; vertical-align: bottom;"></td>
		<td width='33%'>&nbsp;</td>
	</tr>
	...
	<tr>
		<td colspan='3'>
			<table cellpadding="10" cellspacing="3">
				<tr><td colspan='6'></td><td colspan='2'>Date: <?php echo date("d-m-y");?></td></tr>
...
<?php
	//Start: Email form
	echo "<strong>Note:</strong><span style='background-color:yellow'> If email function is not working from chrome browser, please try it from firefox.</span>";
	...
	echo $this->Form->input(null,array('type' => 'hidden','value' => 1,'name' => 'email2Customer'));
