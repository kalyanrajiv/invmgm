...
<table border="1" cellspacing="0" style="width: 750px;">
	...
	<tr>
		<td colspan="6"><strong>Purchase Details</strong></td><td>
	...
	<tr>
		<?php if($productReceipt['bulk_invoice'] != 1){ ?>
		<td style="width: 880px;">
		<?php }else{?>
		<td colspan="2" style="width: 880px;">
		<?php }?>
	...
	<tr>   <?php //removed as requested by the client // we uncommented it on 3 aug ?>
		<td colspan="6"><strong>Sub Total(total qty = <?php echo $totalqty;?>)</strong></td>
		<td><?=$CURRENCY_TYPE.number_format($amount,2);?></td>
	</tr>
	
	<tr>
		<td colspan="6"><strong>Bulk Discount(<?php echo $productReceipt['bulk_discount'];?>%) </strong></td>
		<td><?=$CURRENCY_TYPE.number_format($bulkDiscount,2);?></td>
	</tr>
	
	<tr>
		<td colspan="6"><strong>After Bulk Discount</strong></td>
	...
	if(array_key_exists(0,$dataArr)){//0 is for refund in status?>
		...
		<tr>
		<td colspan="6"><strong>Product Return Details</strong></td>
	</tr>
	<tr>
	...
	<tr>
			...
			<td colspan="2">Fax(Sales) </br><?=$settingArr['fax_number'];?></td>
					...
					<td colspan="1">Website </br><?=$settingArr['website'];?></td>
		</tr>
	</table>
