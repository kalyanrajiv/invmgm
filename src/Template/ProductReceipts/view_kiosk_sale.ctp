...
<h4><?php echo $this->Html->link('Generate Receipt', array('action' => 'generate_receipt',$productReceipt['id'],$kioskId))?></h4>
	<dl>
		...
		<dd>
			<?php if(!empty($customer_data)){echo $this->Html->link($customer_data[0]['id'], array('controller' => 'customers', 'action' => 'view', $customer_data[0]['id']));} ?>
			&nbsp;
		</dd>
...
<div class="related">
	...
	<?php
		...
		$truncatedProduct = \Cake\Utility\Text::truncate($productTitle, 35,['ellipsis' => '...', 'exact' => false]);
	...
	<?php 	} ?>
			...
			<td class="actions">
				<?php if($kioskProductSale['status']==1 && (int)$kioskProductSale['quantity']){
					if($kioskProductSale['sale_type'] == 0){
					echo $this->Html->link(__('Refund Product'), array('controller' => 'kiosk_product_sales', 'action' => 'admin_kiosk_product_refund', 

$kioskId, $kioskProductSale['id'],$productReceipt['id']));	
					}
						...
						<?php
						if($kioskProductSale['sale_type'] == 0){
								echo $this->Html->link(__('Edit Payment'), array('controller' => 'kiosk_product_sales', 'action' => 'edit_payment', 

$kioskId,$productReceipt['id'],$kioskProductSale['id']));
						}
					}
				}else{
					echo "...";
