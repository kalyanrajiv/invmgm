...
<div class="productReceipts index">
	...
	?>
	<?php $webroot = $this->request->webroot."product-receipts/drPaymentClearnessSearch";?>
		...
		<tr>
			<td colspan="5">
				...
				if($this->request->session()->read('Auth.User.group_id') == ADMINISTRATORS || $this->request->session()->read('Auth.User.group_id') == FRANCHISE_OWNER || $this->request->session()->read('Auth.User.group_id') == SALESMAN || $this->request->session()->read('Auth.User.group_id') == inventory_manager){
					...
					?>
				<strong><?php echo __('<span style="color:red; font-size:20px;">'.$kiosk_list[$kiosk].' Quotation Pmt Logs</span>').$this->Html->link(' (view all invoices)',array('action'=>'all_invoices')); ?></strong>
				<?php }else{ ?>
				<strong><?php echo __('<span style="color:red; font-size:20px;">Processed Quotation Payment Logs</span>').$this->Html->link(' (view all invoices)',array('action'=>'all_invoices')); ?></strong>
				<?php } ?>
			</td>
			<?php //saleman add by rajju
	...
	<tr>
		...
		<td><?php echo h($productreceiptArr[$productReceipt->product_receipt_id]['customer_id']); ?>&nbsp;</td>
		...
		<td><?php if(!empty($log_refined_data[$productReceipt->id])){
			 if(empty($users[$log_refined_data[$productReceipt->id][0]->user_id])){
				echo "Missing Name";
			}else{
				echo $users[$log_refined_data[$productReceipt->id][0]->user_id];
			}
			} ?></td>
		<td title="<?php echo $description;?>"><?php if(!empty($log_refined_data[$productReceipt->id])){
			...
			if(array_key_exists($productReceipt->id,$log_refined_data) && count($log_refined_data[$productReceipt->id]) > 1){
				...
				foreach($log_refined_data[$productReceipt->id] as $log_detail_key => $log_detail_val){
					$counting++;
					if(empty($str)){
						$first_name = $users[$log_detail_val->user_id];
						if(empty($first_name)){
							$first_name = "Missing Name";
						}
						$str .= "'".$counting."- ".date("d/m/Y h:i",strtotime($log_detail_val->created)).", ".$first_name.", ".$log_detail_val->old_pmt_method."->".$log_detail_val->pmt_method;
