...
$process_cart = $this->Url->build(['controller' => 'product-receipts', 'action' => 'process_cart'],true);
$process_credit_cart = $this->Url->build(['controller' => 'product-receipts', 'action' => 'process_credit_cart'],true);
?>
...
<input type= "hidden" id="alert_box_value" />
	...
	<thead>
	 <tr>
		...
		<tr>
			<td colspan="3">
			   ...
			   ?>
                ...
                <?php }else{?>
                <strong><?php echo __('<span style="color:red; font-size:20px;">Manage Outstanding For Invoices</span>')?></strong>
                <?php } ?>
			</td>
			<td>
		...
		<td colspan=6>
			<span>
				<b>**Adjustment against credit note will keep payment date same as of Credit Note and method would be Cash always.</b>
			</span>
		</td>
		</tr>
	<tr>
