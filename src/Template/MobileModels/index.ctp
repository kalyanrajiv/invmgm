...
<div class="actions">
    ...
    <ul>
		<li><?php echo $this->Html->link(__('New Mobile Model'), array('action' => 'add')); ?></li>
...
<div class="mobileModels index large-9 medium-8 columns content">
	...
	?>
	<h2><?php echo __('Mobile Models'); ?>&nbsp;<a href='<?php echo $this->request->webroot;?>MobileModels/export/?<?php echo $queryStr;?>' target='_blank' title='export csv'><?php echo $this->Html->image('/img/export.png',array('fullBase' => true));?></a></h2>
	...
	//pr($selectedBrandIds);
	?>
	<form action='<?php echo $this->request->webroot;?>mobile-models/search' method = 'get'>
    ...
    <table cellpadding="0" cellspacing="0">
        ...
        <tbody>
            <?php foreach ($mobileModels as $mobileModel): ?>
            <tr>
                <td><?= $this->Number->format($mobileModel->id) ?></td>
                <td><?= $mobileModel->has('brand') ? $this->Html->link($brands[$mobileModel->brand->id], ['controller' => 'Brands', 'action' => 'view', $mobileModel->brand->id]) : '' ?></td>
                <td><?= h($mobileModel->model) ?></td>
                <td><?= h($mobileModel->brief_description) ?></td>
                <td><?=  $activeOptions[$mobileModel->status] ?></td>
                <?php
                        $created = $mobileModel->created;
                          $created->i18nFormat(
                                                                            [\IntlDateFormatter::FULL, \IntlDateFormatter::FULL]
                                                                    );
                        $created_1 =  $created->i18nFormat('dd-MM-yyyy HH:mm:ss');
                        $modified = $mobileModel->modified;
                         $modified->i18nFormat(
                                                                            [\IntlDateFormatter::FULL, \IntlDateFormatter::FULL]
                                                                    );
                ...
                <td class="actions">
                    <?= $this->Html->link(__('View'), ['action' => 'view', $mobileModel->id]) ?>
                    <?= $this->Html->link(__('Edit'), ['action' => 'edit', $mobileModel->id]) ?>
                    <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $mobileModel->id], ['confirm' => __('Are you sure you want to delete # {0}?', $mobileModel->id)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
