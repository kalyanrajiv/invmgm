...
<div class="kioskOrders index">
	...
	?>
	<form action='<?php echo $this->request->webroot;?>stock-transfer/search_suspense_stock' method = 'get'>
	<?php //echo $this->request->webroot;?>
					...
					<tr><td>
						...
						<tr>
							<td style="width: 425px;">
								<div id='remote'>
									<input type = "text"  class="typeahead" name = "search_kw" id = "search_kw" placeholder = "product code or name" value = '<?= $value;?>'style = "width:200px" autofocus/>
								</div>
							</td>
							<td>
								<select id="site" onchange="this.form.submit()" name="site">
								<?php foreach($sites as $site_id => $site_name){ ?>
										<option <?php if($site == $site_id){echo "selected=selected";} ?> value="<?php echo $site_id;?>"><?php echo $site_name;?></option>
								<?php }?>
								</select>
							</td>
						</tr>
...
<div id='printit'>
	...
	?>
	<strong><?php 
	echo __('<span style="font-size: 20px;color: red;">Suspense Stock</span> <span style="font-size: 17px;"></span>'); ?></strong>
...
endforeach; ?>
    ...
    <?php if(!empty($suspenseData)){ ?>
    <tbody>
        <tr>
            <td colspan=5 style="padding-left: 574px;"> <b> total =  </b></td>
