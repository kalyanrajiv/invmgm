...
<?php echo $this->Form->create('UpdatePayment'); ?>
		...
		<table>
			...
			<tr>
				<?php foreach($res as $s => $k){
						foreach($creditPaymentDetail as $paymentData){ //pr($paymentData);die;?>
							...
							<td>
							<input type="text" id= "<?php echo "old_amt_box_".$paymentData['id']; ?>" name="<?php echo "payment[".$paymentData['id']."]"; ?>" style="width: 100px;" value="<?php echo $paymentData['amount'];?>"/>
	...
	echo $this->Form->Submit('Update Payment',array('name'=>'submit'));
    echo $this->Form->end(); ?>
	</div>
...
<script>
	$('#update_payment').click(function(){
		...
		<?php foreach($res as $s => $k){
			foreach($creditPaymentDetail as $paymentData){
			?>
