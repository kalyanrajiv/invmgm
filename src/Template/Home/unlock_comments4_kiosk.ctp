<h2>Comments(Posted by unlock technicians)</h2>
 <?php
use Cake\Utility\Text;
use Cake\Routing\Router;
  
?>
<span><?php echo $this->Html->link('posted by repair technicians',array('action' => 'repair_comments_4_kiosk'));?></span>
<?php
...
if(count($comments) &&
	...
	<table>
	    <tr>
			<form action='<?php echo $this->request->webroot; ?>home/search_unlock_comments_4_kiosk' method = 'get'>
			<td colspan="5">
		    <table style='width: 50%;'>
			<tr>
				<td><input type = "text" name = "id" id = "id" placeholder = "Unlock Id"  autofocus  value ='<?php echo $id;?>'/></td>
			    ...
			    <td><?php echo $this->Form->input('kiosk', array(
                                                                 'options' => $kiosks,
                                                                 'default' => $kiosk,
                                                                 'label' => false,
                                                               //  'empty' => 'Choose Kiosk',
		...
		?>
	    <tr>
		<td style='width: 160px;color: blue;'><?=date('jS M, Y g:i A',strtotime($comment->created));//$comment->created;?></td>
		<td style='width: 50px; text-align: center;font-weight: bold;'><?=$this->Html->link($comment->mobile_unlock_id,array('controller' => 'mobile_unlocks', 'action'=>'view',$comment->mobile_unlock_id),array('target'=>'_blank'));?></td>
		<td style='width: 140px; color: brown;'><?=$kiosks[$comment->mobile_unlock->kiosk_id];?></td>
		<td style='width: 140px; color: darkviolet;'><?=$unlockusers[$comment->user_id];?></td>
