...
<?php
    ...
    //$siteBaseURL = Configure::read('SITE_BASE_URL'); //rasu
     ...
     $priceURL = $this->Url->build(["controller" => "home","action" => "cr_get_products_json_by_title"],true);
	$priceTitleURL = $this->Url->build(['controller' => 'home', 'action' => 'cr_get_products_json_by_title'],true);
	$save_invoice = $this->Url->build(['controller' => 'home', 'action' => 'save_invoice'],true);
	$restore_session_db = $this->Url->build(['controller' => 'home', 'action' => 'cr_restore_session_db'],true);
	$update_bulk = $this->Url->build(['controller' => 'home', 'action' => 'cr_update_bulk'],true);
	
	$add_2_cart_full = $this->Url->build(['controller' => 'home', 'action' => 'cr_add_2_cart_full'],true);
	$update_cart = $this->Url->build(['controller' => 'home', 'action' => 'cr_update_cart'],true);
	$delte_cart = $this->Url->build(['controller' => 'home', 'action' => 'cr_delete_from_cart'],true);
	$cartURL = $add_2_cart_short = $this->Url->build(['controller' => 'home', 'action' => 'cr_add_2_cart_short'],true);
	$restore_cart = $this->Url->build(['controller' => 'home', 'action' => 'cr_restore_cart'],true);
	$clear_cart = $this->Url->build(['controller' => 'home', 'action' => 'cr_clear_cart'],true);
	$update_type = $this->Url->build(['controller' => 'home', 'action' => 'cr_update_type'],true);
    
    
	
?>
<div class="index" style='width: 97%; background-color: #FFE4E1; padding-left: 10px;'>
    <h1 style='color:#6B8E23;  font-size: 250%; background-color: #FFE4E1;'>&raquo;Create Credit</h1>  
    <table width='100%'>
    <tr>
        <td width='50%'>
						...
						<table style="font-size: 10px;">
							...
							<tr>
								...
								$loggedInUser = $this->request->session()->read('Auth.User.username');
								 if($this->request->session()->read('Auth.User.group_id') == ADMINISTRATORS || $this->request->session()->read('Auth.User.group_id') == FRANCHISE_OWNER || $this->request->session()->read('Auth.User.group_id') == SALESMAN || $this->request->session()->read('Auth.User.group_id') == inventory_manager ){ //saleman add by rajju
		...
		<?php echo $this->element('/home/cr_payment'); ?>
...
<input type='hidden' name='update_type' id='update_type' value='<?=$update_type?>' />
...
<script type="text/javascript">
	...
	var product_dataset = new Bloodhound({
		...
		remote: {
		   url: "/home/adminDataWQty?category=%CID&search=%QUERY",
	...
	function show_cart(objArr) {
		...
		var innerHTMLStr = "<table width='100%' cellspacing='2' cellpadding='2'><tr><th style='width:30px;'>Del</th><th>Code</th><th>Title</th><th>Unit Price</th><th>Qty</th><th>type</th><th>Amount</th><th>Edit</th></tr>";
		var resultFound = false;
		var total_amount = 0
		...
		for (i = 0; i < posArr.length; i++) {
			...
			$.each(itemObj, function(key, obj){
				...
				if (position == parseInt(obj.position)) {
					...
					innerHTMLStr += "<td><select id = 'type_"+key+"' onchange='update(type_"+key+")'>";
					if (obj.type == "normal") {
						var selected_n = " selected = selected";
					}else{
						var selected_n = "";
					}
					if (obj.type == "faulty") {
						var selected_f = " selected = selected";
					}else{
						var selected_f = "";
					}
					
					 innerHTMLStr +=  "<option value=normal"+selected_n+">Normal</option>";
					innerHTMLStr +=  "<option value=faulty"+selected_f+">Faulty</option>";
					//innerHTMLStr += "<td><input type='text' id='type"+key+"' value=normal /></td>";
...
//		}
	...
	function  update(name) {
        var selected_val = name.value;
		var id = name.id;
		 var sln = id.length; 
		 var id_to_update = id.substring(id.lastIndexOf("_")+1,sln);
		 targeturl = $("#update_type").val();
		 targeturl += "?id="+id_to_update+"&";
		 targeturl += "value="+selected_val;
		$.blockUI({ message: 'Updating cart...' });
		$.ajax({
			type: 'get',
			url: targeturl,
...
//	});
	...
	$(document).on('click', '.update_cart', function() {
		...
		var type =$('#type_'+product_id+' :selected').val();
		var recit_req = 0;
...
<script>
	...
	function check_sp_na(prod_id) {
		...
		if (parseFloat(sell_p_val) < 0) { //parseFloat(dis_val)
			...
			$( "#dialog-selling_amount" ).dialog({
				...
				width:300,
				maxWidth:300,
				title: '!!! selling amount cannot be less then Zero!!!',
				buttons: {
					"OK": function() {
						$( this ).dialog( "close" );
					}
