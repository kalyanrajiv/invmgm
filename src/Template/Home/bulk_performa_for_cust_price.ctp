...
<div class="index" style='width: 97%; background-color: #F0F8FF; padding-left: 10px;'>
    <h1 style='color:blue;  font-size: 250%; background-color: #F0F8FF;'>&raquo;Create Performa</h1>
    <table width='100%'>
    <tr>
        <td width='50%'>
		<table width='100%'>
		...
		<?php echo $this->element('/home/pf_payment'); ?>
        <td width='50%'>
				...
				success: function(response) {
					...
					$.each(objArr, function(key, obj){ //str.concat();
						...
						if (dis_status == 1) {
							...
							innerHTMLStr += "<td style='width:35px;'><input type='text' name='sp' title="+parseFloat(obj.minimum_selling_price).toFixed(2)+ " value='"+obj.price_without_vat+"' style='width:35px;'  id='sp_"+obj.id+"' onchange='check_sp_na("+obj.id+")'/></td>";
						}
