...
<?php
    ...
    $webRoot = $this->request->webroot; //FULL_BASE_URL.
	...
	$save_invoice = $this->Url->build(['controller' => 'home', 'action' => 'save_invoice'],true);
	$restore_session_db = $this->Url->build(['controller' => 'home', 'action' => 'restore_session_db'],true);
	$update_bulk = $this->Url->build(['controller' => 'home', 'action' => 'update_bulk'],true);
	
	$add_2_cart_full = $this->Url->build(['controller' => 'home', 'action' => 'add_2_cart_full'],true);
	$update_cart = $this->Url->build(['controller' => 'home', 'action' => 'update_cart'],true);
	$delte_cart = $this->Url->build(['controller' => 'home', 'action' => 'delete_from_cart'],true);
	$cartURL = $add_2_cart_short = $this->Url->build(['controller' => 'home', 'action' => 'add_2_cart_short'],true);
	$restore_cart = $this->Url->build(['controller' => 'home', 'action' => 'restore_cart'],true);
	$clear_cart = $this->Url->build(['controller' => 'home', 'action' => 'clear_cart'],true);
	 //$priceURL = $this->Html->url[['controller' => 'home', 'action' => 'get_products_json_by_title']];die;
	//$priceTitleURL = $this->Html->url[['controller' => 'home', 'action' => 'get_products_json_by_title']];
	//$save_invoice = $this->Html->url[['controller' => 'home', 'action' => 'save_invoice']];
	//$restore_session_db = $this->Html->url[['controller' => 'home', 'action' => 'restore_session_db']];
	//$update_bulk = $this->Html->url[['controller' => 'home', 'action' => 'update_bulk']];
	//
	//$add_2_cart_full = $this->Html->url[['controller' => 'home', 'action' => 'add_2_cart_full']];
	//$update_cart = $this->Html->url[['controller' => 'home', 'action' => 'update_cart']];
	//$delte_cart = $this->Html->url[['controller' => 'home', 'action' => 'delete_from_cart']];
	//$cartURL = $add_2_cart_short = $this->Html->url[['controller' => 'home', 'action' => 'add_2_cart_short']];
	//$restore_cart = $this->Html->url[['controller' => 'home', 'action' => 'restore_cart']];
	//$clear_cart = $this->Html->url[['controller' => 'home', 'action' => 'clear_cart']];
?>
<div class="index" style='width: 97%; background-color: #FFFFF0; padding-left: 10px;'>
    <h1 style='color:#F08080;  font-size: 250%; background-color: #FFFFF0;'>&raquo;Create Invoice</h1>
    <table width='100%'>
    <tr>
        <td width='50%'>
                    ...
                    <legend>Search</legend>
						...
						<table style="font-size: 10px;">
							<tr>
								...
								<td style="width: 95px"><?= $customer['id'] ?>
							...
							<tr>
								...
								$loggedInUser = $this->request->session()->read('Auth.User.username');
								 if(true){  //AuthComponent::user('group_id') == ADMINISTRATORS
									if(preg_match('/dr5/',$loggedInUser)){ ?>
										<td>
											<table style='width:190px;'>
												<tr>
													<td>Quotation?</td>
													<?php if($customer['id'] == 66){?>
													<td>Yes<input type='radio' name='special_invoice' value='1' checked='checked'/></td>
													<td>No<input type='radio' name='special_invoice' value='0' /></td>
													<?php }elseif($customer['country'] == "OTH"){ ?>
														<td>Yes<input type='radio' name='special_invoice' value='1'/></td>
													<td>No<input type='radio' name='special_invoice' value='0' checked='checked'/></td>
													<?php }else{?>
													<td>Yes<input type='radio' name='special_invoice' value='1'checked='checked' /></td>
													<td>No<input type='radio' name='special_invoice' value='0' /></td>
													<?php }?>
									...
									$type = AuthComponent::user('user_type');
									if($type == "wholesale"){
										if(preg_match('/dr5/',$loggedInUser)){ ?>
											<td>
												<table style='width:190px;'>
		...
		<?php echo $this->element('/home/payment'); ?>
...
<script type="text/javascript">
				...
				success: function(response) {
					...
					$.each(objArr, function(key, obj){ //str.concat();
						...
						if (dis_status == 1) {
							...
							innerHTMLStr += "<td><input type='text' name='sp'    class='validate_qty' value='"+obj.price_without_vat+"' style='width:35px;' id='sp_"+obj.id+"' onchange='check_sp("+obj.id+")'/></td>";
                        }else{
							innerHTMLStr += "<input type='hidden' id='dis_"+obj.id+"' value="+parseFloat(obj.minimum_selling_price).toFixed(2)+" />";
							innerHTMLStr += "<td>N/A</span></td>";
							innerHTMLStr += "<td style='width:35px;'><input type='text'   class='validate_qty1' name='sp' title="+parseFloat(obj.minimum_selling_price).toFixed(2)+ " value='"+obj.price_without_vat+"' style='width:35px;'  id='sp_"+obj.id+"' onchange='check_sp_na("+obj.id+")'/></td>";
						}
	...
	function show_cart(objArr) {
		...
		for (i = 0; i < posArr.length; i++) {
			...
			$.each(itemObj, function(key, obj){
				...
				if (position == parseInt(obj.position)) {
					...
					if (min_dis == 0 || min_dis == obj.selling_price) {
						...
						innerHTMLStr += "<td><input type='text' title = '"+title+"' class='validate_qty1' name='sp' value='"+obj.selling_price+"' style='width:50px;' id='cart_sp_"+key+"' onchange='check_sp_cart("+key+")'/></td>";
					}
		...
		if (parseFloat(total_amount) > 0) {
			if (country == "OTH") {
				innerHTMLStr += "<tr><td><a href='#-1' class = 'make_payment' rel='"+total_amount+"'>make payment</a></td></tr>";
            }else{
				innerHTMLStr += "<tr><td><a href='#-1' class = 'make_payment' rel='"+final_amt+"'>make payment</a></td></tr>";
			}
...
//		}
	...
	$(document).on('click', '.make_payment', function() {
        ...
        //var amt = roundNumber(amt,2);
		var bulk_amt = $('#bulk_discount').val();
		if (bulk_amt != "") {
            var bulk_dis_val = amt * (bulk_amt/100);
			//alert(bulk_dis_val);
			amt = amt - bulk_dis_val;
        }
		amt = parseFloat(amt).toFixed(2);
		$("#invoice_amt").text(parseFloat(amt).toFixed(2));
		document.getElementById('total_hidden_amount').value = amt;
		document.getElementById('total').value = amt;
		document.getElementById('final_amount').value = amt;
		$('#payment_method_0').val(amt);
        $('#payment_method_1').val("");
        $('#payment_method_2').val("");
		$('#bulk_discount_input').val(bulk_amt);
		$('#paymentDiv').show();
	});
...
<script>
	...
	$(function() {
        ...
        $(".validate_qty1").keydown(function (event) { 
                if (event.shiftKey == true) {event.preventDefault();}
                if ((event.keyCode >= 48 && event.keyCode <= 57) ||
                   (event.keyCode >= 96 && event.keyCode <= 105) ||
                   event.keyCode == 8 || event.keyCode == 9 ||
