<?php
$login = $this->request->session()->read('Auth.User.id');
//pr($_SESSION);die;
$server = $_SERVER['SERVER_NAME'];
    $Server = explode('.',$server);
    //pr($Server);die;
    if($Server[0] == 'mbwaheguru' || $Server[0] == 'hifiprofile' || $Server[0] == 'fonerevive'){
        $name = $Server[0];
    }else{
        $name = $Server[0].'.'.$Server[1];
    }
if($login){
    if($Server[0] == 'mbwaheguru'){
        header("Location: http://$name.co.uk/home/dashboard");    
    }else{
        header("Location: http://$name.net/home/dashboard");
    }
    
}else{
    if($Server[0] == 'mbwaheguru'){
        header("Location: http://$name.co.uk/users/login");    
    }else{
        header("Location: http://$name.net/users/login");
    }
}
die;
?>
