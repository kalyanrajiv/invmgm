        ...
        foreach($kiosk_placed_user_date1 as $key1 => $value1){
            ...
            if(!empty($value1)){
               ...
               // $value_date1 = date("d-m-y h:i a",strtotime($value1));
                $kiosk_placed_user_date[$key1] = $value1;
            }else{
                $value1 = "--";
                $kiosk_placed_user_date[$key1] = $value1;
            }
...
//                }
		...
		$kiosk_placed_user_id_query = $this->KioskPlacedOrders->find('list',array(
																		'conditions' =>['id IN'  => $kiosk_placed_orderid],
																		 'keyField' => "id",
																		'valueField' => "user_id",
																			  ));
		...
		$kiosk_placed_user_date_query = $this->KioskPlacedOrders->find('list',array(
         ...
         $kiosk_placed_user_date_query =  $kiosk_placed_user_date_query->hydrate(false);
		...
		if(!empty($kiosk_placed_orderid)){
		  ...
		  $kiosk_placed_merged_orders = "";
		}
		
		$hint = $this->ScreenHint->hint('kiosk_orders','transient_orders');
		if(!$hint){$hint = "";}
		$this->set(compact('orderOptions'));
		$this->set(compact('hint','kioskOrders','users','kiosks','kiosk_id','kiosk_placed_user_date','kiosk_placed_user_id','kiosk_placed_merged_orders','on_demand_placed_user_id','on_demand_user_date'));
    ...
    public function revertStock($kioskOrderID = '',$kioskID =''){
		...
		$isboloRam = false;
               foreach($sites as $site_name => $site_path){
                       $isboloRam = strpos($path,$site_path);
                       if($isboloRam){                       
                               break;
                       }
               }
       ...
       $kiosks_query = $this->Kiosks->find('list',[
        ...
        foreach($on_demand_placed_user_date1 as $key => $value){
            ...
            if(!empty($value)){
                ...
                $on_demand_placed_user_date[$key] = $value;
            }else{
                $value_date = "--";
...
//                pr($productId);die;
		...
		$this->set(compact('orderOptions'));
		$this->set(compact('hint','kioskOrders','users','kiosks','kiosk_id','kiosk_placed_user_date','kiosk_placed_user_id','kiosk_placed_merged_orders','on_demand_placed_user_date','on_demand_placed_user_id'));
    ...
    public function searchFromOrders(){
      ...
      $kioskOrderIdArr = array();
		...
		$kioskOrderedProducts_query = $this->KioskOrderProducts->find('all',array('conditions'=>array('KioskOrderProducts.kiosk_placed_order_id IN'=>$kioskPlacedOrderIds),'fields'=>array('kiosk_placed_order_id','product_id','quantity','org_qty')));
		...
		foreach($kioskOrderedProducts as $key=>$kioskOrderedProduct){
			...
			$kioskOrderedProductDetail[$newKey][$kioskOrderedProduct['product_id']] = $kioskOrderedProduct['org_qty'];
		}
		...
		if($type == "received_less"){
				...
				foreach($kioskOrderIdArr as $k=>$kioskOrderId){
					...
					if(array_key_exists($kioskOrderId,$dispatchedproductsArr)){
						foreach($dispatchedproductsArr[$kioskOrderId] as $compProductId=>$qtt){
							...
							if(array_key_exists($kioskOrderId,$kioskOrderedProductDetail) && array_key_exists($compProductId,$kioskOrderedProductDetail[$kioskOrderId])){
		 ...
		 $external_sites = Configure::read('external_sites');
		  ...
		  if($ext_site == 1){
			   $managerKiosk = $this->get_kiosk();
			   if(!empty($managerKiosk)){
				  if($receiptTable_source == "stock_transfer_by_kiosk"){
						   $this->paginate = [
                                    'conditions' => [
...
//                                              ];
	...
	public function changeTransientOrderKiosk(){
		 if(array_key_exists("order_id",$this->request->data)){
				  if(!empty($this->request->data["order_id"])){
						   $order_id = $this->request->data["order_id"];
				  }else{
						   $msg = array("msg" => "No Order Id Found");	   	   
				  }
		 }else{
				  $msg = array("msg" => "No Order Id Found");	   	   
		 }
		 
		 if(array_key_exists("selected_kiosk_id",$this->request->data)){
				  if(!empty($this->request->data["selected_kiosk_id"])){
						   $selected_kiosk_id = $this->request->data["selected_kiosk_id"];
				  }else{
						   $msg = array("msg" => "No kiosk Id Found");	   	   
				  }
		 }else{
				  $msg = array("msg" => "No kiosk Id Found");	   	   
		 }
		 
		 if(array_key_exists("kiosk_placed_order_id",$this->request->data)){
				  if(!empty($this->request->data["kiosk_placed_order_id"])){
						   $kiosk_placed_order_id = $this->request->data["kiosk_placed_order_id"];
				  }else{
						   $kiosk_placed_order_id = "";
				  }
		 }
		 
		 
		 if(!empty($kiosk_placed_order_id)){
				  // update kiosk_orders and kiosk_order_products table and stock transfer
				  $kiosk_order_res = $this->KioskOrders->find("all",[
												  "conditions" => [
													  "id" => $kiosk_placed_order_id,
												  ]
												  ])->toArray();
				  $kiosk_order_query = "UPDATE `kiosk_orders` SET `kiosk_id` = $selected_kiosk_id WHERE `id` = $order_id";
				  
				  $conn = ConnectionManager::get('default');
				  $stmt = $conn->execute($kiosk_order_query);
				  
				  $kiosk_order_product_query = "UPDATE `kiosk_order_products` SET `kiosk_id` = $selected_kiosk_id WHERE `kiosk_placed_order_id` = $order_id";
				  
				  $stmt = $conn->execute($kiosk_order_product_query);
				  
				  $kiosk_order_product_query = "UPDATE `stock_transfer` SET `kiosk_id` = $selected_kiosk_id WHERE `kiosk_order_id` = $order_id";
