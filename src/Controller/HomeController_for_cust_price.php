...
class HomeController extends AppController{
    ...
    public function getProductsJsonByTitle(){
		...
		foreach($prodRecs as $prodRec){
			...
			$prodRec['minimum_selling_price'] = round($priceWithoutVat,2);
			//--------price without vat and discounted value---------
			if(!empty($cust_id)){
				$customer_base_price_list = array();
				$customer_base_price_list = $this->CustomerProductPrice->find("list",[
				...
				if(!empty($customer_base_price_list) && array_key_exists($prodRec['id'],$customer_base_price_list)){
					if($prodRec['minimum_selling_price'] > $customer_base_price_list[$prodRec['id']]){
						// do nothing
					}else{
						$priceWithoutVat = $customer_base_price_list[$prodRec['id']];
						$prodRec['price_without_vat'] = round($priceWithoutVat,2);	
					}
				}
		...
		if(is_array($quick_cart) && count($quick_cart) > 0){
			...
			if(count($prodRow) >= 1 ){
				...
				if(array_key_exists($prod_id, $items)){
					...
					if(!empty($customer_base_price_list)){
						if($customer_base_price_list[$prodRow['id']] < $netVal){
							// do nothing
						}else{
							$priceWithoutVat = $customer_base_price_list[$prodRow['id']];
						}
					}
           ...
           // pr($session_basket);die;
			foreach($session_basket as $key => $value){
				...
				if($kiosk_saleTable->save($kiosk_saleDetails)){
					...
					$sale_price = $priceWithoutVat - $priceWithoutVat * ($discountPercentage/100);
					
					//$this->AddEditCustomerProductPrice($customer_id,$key,$sale_price);
...
//    }
	...
	public function newProductsNotification(){
		...
		//--------- Code below for notification of update in quantity of warehosue products
		$this->set(compact( 'currency','productNofification','warehouseProductNotification','warehouseProductNotificationArr','categoryNames'));
		//$this->set(compact('notificationStatement','currency','productNofification','warehouseProductNotification','mobileRepairPriceNotification','mobileUnlockPriceNotification'));
		...
		if(!empty($session_basket)){
			foreach($session_basket as $key => $value){
				...
				if($CreditProductDetailTable->save($patchEntity)){
					...
					//$this->AddEditCustomerProductPrice($customer_id,$key,$sold_price);
					//Start: newly added by rajiv on Oct 10, 2016
					if($kiosk_id < 10000 && $kiosk_id != 0 && !empty($kiosk_id)){
						$productTable = "kiosk_{$kiosk_id}_products";
					}else{
		...
		foreach($basket as $key => $value){
			...
			$quantity = $value['quantity'];
				...
				if($InvoiceOrderDetailTable->save($patch_entity)){
					//$this->AddEditCustomerProductPrice($cust_info['id'],$product_id,$sold_price);
					$count++;
				}else{
	...
	public function freshArrival(){
		...
		$this->set(compact('currency','productNofification','basketStr','session_basket'));		
	}
