...
//                         ];
			...
			if(!empty($warehouseBasket) && !empty($reference_number)){
				...
				foreach($warehouseBasket as $productId=>$productSession){
					...
					$product_query = $this->Products->find('all',array(
																	'fields' => array('qty_update_status','qty_update_time','quantity','selling_price','retail_selling_price'),
					...
					if($productSession['in_out'] == 1){
                        ...
                        $with_vat_Selling_price = $productSession['new_selling_price'] + $productSession['new_selling_price'] * (($this->VAT)/100);
						$change = 0;
							if($oldPrdctInfo[$productId]['cost_price'] != $productSession['price'] ||
								 $oldPrdctInfo[$productId]['selling_price'] != $with_vat_Selling_price ||
								 $oldPrdctInfo[$productId]['retail_cost_price'] != $productSession['new_rcp'] ||
								 $oldPrdctInfo[$productId]['retail_selling_price'] != $productSession['new_rsp']
								 ){
							  $change = 1;
							}
						$this->Products->updateAll(
													array(
															...
															'products.selling_price' => $with_vat_Selling_price,//$productSession['new_selling_price'],
					...
					$settingArr = $this->Settings->find("list",[
						...
						$setting_qty = $settingArr['bk_stk_in_lvl'];
						...
						if($qty_update_status == 0){
						 ...
						 $query = "UPDATE `products` SET modified='$currentDate',quantity = quantity + '$productSessionQuantity',`qty_update_status` = 1, `qty_update_time` = NOW() WHERE id='$productId'";
							  ...
							  if($current_date > $date){
								   ...
								   if($prevQuantity < $setting_qty){
										if($warehouseData['current_stock'] > $setting_qty){
											 $query = "UPDATE `products` SET modified='$currentDate',`qty_update_status` = 2,quantity = quantity + '$productSessionQuantity',`back_stock_status` = 1, `back_stock_time` = NOW() WHERE id='$productId'";		 	  
										}else{
							  ...
							  // this case will run when product is out of fresh arrival screen
							  $setting_minimum_qty = $settingArr['min_bck_stk_in_qty'];
							  if($prevQuantity < $setting_qty){
								   if($warehouseData['current_stock'] > $setting_qty){
										if($productSessionQuantity >= $setting_minimum_qty){
										$query = "UPDATE `products` SET modified='$currentDate',`back_stock_status` = 1, `back_stock_time` = NOW(),quantity = quantity + '$productSessionQuantity' WHERE id='$productId'";   	 
										}else{
											 $query = "UPDATE `products` SET modified='$currentDate',quantity = quantity + '$productSessionQuantity' WHERE id='$productId'";		   
										}
								   }else{
						...
						if($this->WarehouseStock->save($WarehouseStock,['validate' => false])){
						 if($selling_price != $with_vat_Selling_price){
							  $update_query = "UPDATE `stock_transfer` SET `sale_price` = $with_vat_Selling_price WHERE `product_id` = $productId";
							  $stmt = $conn->execute($update_query);
						 }
						 if($productSession['new_rsp'] != $retail_selling_price){
							  $retail_product[$productId] = $productSession['new_rsp'];
						 }
						...
						if(($prevQuantity - $productSession['quantity']) >= 0){						
							...
							$this->Products->updateAll(
									...
									'products.cost_price' => $productSession['price'],
									'products.selling_price' => $with_vat_Selling_price,
									'products.retail_cost_price' => $productSession['new_rcp'],
									'products.retail_selling_price' => $productSession['new_rsp']),
									array('Products.id' => $productId)
								);
					...
					$cost_price = $productSession['price'];
					$selling_price = $with_vat_Selling_price;
					$retail_cost_price = $productSession['new_rcp'];
					$retail_selling_price = $productSession['new_rsp'];
					
					
					foreach($kiosks as $key => $kiosk_id){
						 $kiosk_product_update_query = "UPDATE kiosk_{$kiosk_id}_products SET cost_price = $cost_price, selling_price = $selling_price, retail_cost_price= $retail_cost_price, retail_selling_price= $retail_selling_price WHERE id = $productId";
						 $conn = ConnectionManager::get('default');
                             $stmt = $conn->execute($kiosk_product_update_query);
					}
					...
					$path = dirname(__FILE__);
						  ...
						  if($isboloRam){
							  if($change == 1){
								 $this->update_price_on_other_site($cost_price,$selling_price,$retail_cost_price,$retail_selling_price,$productId,$productSession['new_selling_price']);  
							  }  
						  }
						//ends
				}
				
				if(!empty($retail_product)){
					$sites = Configure::read('sites');
					if(!empty($sites)){
						 foreach($sites as $k => $val){
							 $conn = ConnectionManager::get($val);
							 foreach($retail_product as $p_id => $rtail_price){
								   $update_query = "UPDATE `stock_transfer` SET `sale_price` = $rtail_price WHERE `product_id` = $p_id";
								    $stmt = $conn->execute($update_query);
						     }
						 }
					}
				...
				if(empty($failedStock)){
					if($duplicate_order == true){
						 $path = dirname(__FILE__);
						  $isboloRam = strpos($path,ADMIN_DOMAIN);
						  if($isboloRam){
							  $this->duplicate_order($warehouseBasket,$reference_number);	 	  
						  }
	...
	public function viewReferenceStock($modifiedDate = ""){
		...
		if(count($productIDs) > 0){
			$products_query = $this->Products->find('all',array(
											  'conditions' => array('Products.id IN' => $productIDs),
											  'fields' => array('product','product_code','id','cost_price')
											  )
								 );
	...
	public function duplicate_order($warehouseBasket,$reference_number){
		  $sites = Configure::read('sites');
		  $warehouse_vendor_id = $this->request->Session()->read('warehouse_vendor_id');
		  foreach($sites  as $key => $value){
			   $conn = ConnectionManager::get($value);
			   $this->WarehouseStock->connection($conn);
			   ...
			   foreach($warehouseBasket as $productId=>$productSession){
					...
					//starts
					$productSession['price'] = $productSession['new_rcp'];
						$productSession['new_selling_price'] = $productSession['new_rsp'];
						$productSession['new_rcp'] = 0;
						$productSession['new_rsp'] = 0;
					$warehouseData = array(
											'quantity' => $productQuantity,
	...
	public function update_price_on_other_site($cost_price,$selling_price,$retail_cost_price,$retail_selling_price,$productId,$without_vat_price){
		    ...
		    foreach($sites  as $key => $value){
			   $conn = ConnectionManager::get($value);
			    $this->Products->connection($conn);
				 $this->Kiosks->connection($conn);
				 
				if(in_array($value,$EXT_RETAIL)){
					
					$this->Products->updateAll(
									array( //'products.quantity' => "products.quantity - $productSessionQuantity",
									'products.cost_price' => $without_vat_price,
									'products.selling_price' => $retail_selling_price,
									'products.retail_cost_price' => $without_vat_price,
									'products.retail_selling_price' => $retail_selling_price),
									array('Products.id' => $productId)
								);
				}else{
					$this->Products->updateAll(
									array( //'products.quantity' => "products.quantity - $productSessionQuantity",
									'products.cost_price' => $retail_cost_price,
									'products.selling_price' => $retail_selling_price,
									'products.retail_cost_price' => $retail_cost_price,
									'products.retail_selling_price' => $retail_selling_price),
									array('Products.id' => $productId)
								);	
				}
			   ...
			   foreach($kiosks as $key => $kiosk_id){
					$kiosk_product_update_query = "UPDATE kiosk_{$kiosk_id}_products SET cost_price = $retail_cost_price, selling_price = $retail_selling_price, retail_cost_price= $retail_cost_price, retail_selling_price= $retail_selling_price WHERE id = $productId";
					$conn = ConnectionManager::get($value);
						$stmt = $conn->execute($kiosk_product_update_query);
			   }
			}
